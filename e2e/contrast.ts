 ...
 *    unknown backdrop. `src/style.css` deliberately declares NONE: every
 *    surface, ink and state fill in this lab is a flat token, so the verdict
 *    pills, the sector tiles, the block cells and the watermark grid are all
 *    resolvable. The `color-mix()` on this page comes entirely from the shared
 *    top bar in `index.html`, which computes its ink as
 *    `color-mix(in srgb, var(--accent) 60%, #eafff8)` and draws its bottom
 *    border and `.cl-btn` hover fill the same way. Those are exactly the
 *    surfaces axe files under `incomplete` rather than judging, which is why
 *    `scan()` asserts that bucket instead of skimming it.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab's stylesheet
 *    contains no `opacity` declaration at all: muted text lowers its own
 *    lightness (`--text-dim`, `--text-muted`) rather than fading, which is the
 *    fleet rule this page was authored to. The opacity model below therefore
 *    measures nothing here today. It stays, and runs at every driven state, so
 *    that remains a measurement rather than a reading of the stylesheet — the
 *    day someone reaches for `opacity: .7` on a disabled control or a
 *    de-emphasised row, this is what judges the colour that actually lands.
 *
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
     * A driven pass here walks one long single-page document, and the
     * expensive parts are the grids: sixteen `.sector-tile` buttons, a
     * thirty-two cell `.block-strip`, act 4's watermark grid at 512 `.wm-cell`
     * spans once two disk images exist, and three `table.matrix` bodies — all
     * of them siblings re-walking the same ancestors up to `<body>`. Without the caches the pass re-reads the same computed
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // that paints nothing. `checkVisibility()` catches it. This page has four
      // of the shape, all shipping shut: the block-independence note in act 2,
      // the design-consequence note in act 4, the Stream Ward cross-link in
      // act 5, and the standards list in the scope card. The gate opens each of
      // them by clicking its <summary>, which is the route a reader has, rather
      // than setting `.open` from script — which is what the gate this replaces
      // did, to every <details> on the page, before its only scan.
     ...
     *
     * Every `aria-hidden` element on this page is a decorative glyph beside
     * its own word: the ✓ / ✕ / ↺ / ● inside each `.verdict-glyph`, and the
     * three `.legend-swatch` marks in act 4's key — plus the shared header's
     * two SVG marks, which carry no text. Each glyph duplicates the word
     * directly next to it, but each is painted in a semantic ink
     * (`--ok-ink`, `--alarm-ink`, `--info-ink`) on a filled pill, which is why
     * `scan()` runs the whole `aria-hidden` set through this walk with the
     * exemption lifted rather than arguing any of them is merely decorative.
     *
     * The 512 watermark cells are deliberately NOT in that set: each is
     * `role="img"` with an `aria-label` naming its sector, its block and what
     * happened to it, because the glyph there is the only carrier of a
     * per-block result and hiding it would delete the act for a screen reader.
     *
     * Nothing on this page hides a VALUE. No tweak, ciphertext, verdict word,
     * failure code or counter is inside an `aria-hidden` subtree — which was
     * checked rather than assumed, because that is the shared blind spot where
     * both oracles stop looking and it is the one place a live readout can hide
