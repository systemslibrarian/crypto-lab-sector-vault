 ...
 * exactly one boundary token — `--border`, `#6b7d92` — applied to every
 * control and every card, and chosen by arithmetic rather than by eye: it
 * clears 3:1 against all four surfaces in the palette (`--bg`, `--surface`,
 * `--surface-2`, `--inset`), so no element can acquire an edge that fails.
 * But tokens are discarded by overrides, and that is what this oracle exists
 * to measure rather than trust. Four shapes on this page override it:
 * `button.primary` and `button.danger` repaint border AND fill the same
 * colour, so they have no edge of their own and live or die by
 * fill-vs-surround; the `.fill-ok` / `.fill-alarm` / `.fill-info` verdict
 * pills, legend swatches and watermark cells do the same; a selected
 * `.sector-tile` or `.block-cell` swaps its border to `--accent` and thickens
 * it; and an `input[aria-invalid="true"]` swaps to `--alarm`. This oracle
 * judges each control as painted, at every driven state — including the
 * `aria-invalid` recolouring and the selected-cell states only the drive
 * reaches.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * expensive parts are the grids: sixteen sector tiles, a thirty-two cell
     * block strip, act 4's 512-cell watermark grid once two disk images exist,
     * and three matrix tables — all siblings re-walking the same ancestors up
     * to `<body>`. Without the
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // Nothing in this lab is delineated by a single side today — every
      // control draws all four, and a selected sector tile or block cell
      // thickens all four together. The per-side walk is what keeps that an
      // implementation detail rather than a load-bearing assumption.
      //
      ...
      // does not currently use that idiom — its selected states thicken a
      // painted border rather than colouring in a transparent one — but
      // counting a transparent spacer as a border is how an element with no
      // fill and no painted edge, identified by its text alone (exactly the
      // case the "is it trying to draw itself as a control?" test below exists
      // to exclude), reports 1.00:1.
